...
async def upload_files(files: List[UploadFile] = FastAPIFile(...)):
    ...
    if not _current_project:
        raise HTTPException(status_code=400, detail="No project is open. Open or create a project first.")

    src_dir = os.path.join(_current_project, "src")
    ...
    for upload in files:
        ...
        _add_log("Upload", f"Uploaded: {upload.filename} ({len(content)} bytes)", "success")
