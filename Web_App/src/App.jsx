...
function OverviewPanel({ t, recentProjects, onOpenProject, onCreateProject, onUploadSuccess, loading }) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: t.panelBg }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '10px 14px' }}>
                ...
                {/* Built-in Samples */}
                <div style={{ fontSize: 10, color: t.mutedText, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6, marginTop: 12 }}>Examples</div>
                ...
                }} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', background: t.cardBg, border: `1px solid ${t.divider}`, borderRadius: 4, cursor: 'pointer', color: t.panelText, fontSize: 12, textAlign: 'left' }}>
                    <FolderOpen size={13} style={{ color: '#dcb67a', flexShrink: 0 }} />
                    <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 500 }}>Load Bank Demo Code</div>
                        <div style={{ fontSize: 10, color: t.mutedText }}>Provisions a sandbox with 4 COBOL files</div>
                    </div>
                </button>
            </div>
        </div>
...
function UploadPanel({ t, onUploadSuccess }) {
    const fileInputRef = useRef(null);
    ...
    const handleFiles = async (files) => {
        ...
        try {
            ...
            if (onUploadSuccess) {
                setTimeout(() => onUploadSuccess(res), 500);
            }
        } catch (e) {
...
export default function App() {
    ...
    const handleRunAnalysis = useCallback(async (overridePath) => {
        const path = (typeof overridePath === 'string') ? overridePath : projectPath;
        if (!path) return;
        setAnalyzing(true);
        try {
    ...
    const handleUploadSuccess = useCallback(async (res) => {
        setProjectPath(res.project_path);
        setProjectName(`Session: ${res.session_id.substring(0, 8)}`);

        // Fetch file tree for the new session
        try {
            const files = await api.listFiles(res.project_path);
            setFileTree(files.tree || []);
        } catch (e) {
            console.error('List files failed:', e);
        }

        setOpenPanel(null); // Auto-hide left panel to show analysis
    }, [handleRunAnalysis]);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', background: t.appBg, color: t.panelText, fontFamily: "system-ui, -apple-system, sans-serif" }}>
            ...
            <header style={{ height: 48, background: t.topBar, borderBottom: `1px solid ${t.divider}`, display: 'flex', alignItems: 'center', padding: '0 16px', gap: 12, flexShrink: 0, zIndex: 50 }}>
                ...
                {/* Cloud Status Badge */}
                <div style={{ display: 'flex', background: t.cardBg, borderRadius: 4, border: `1px solid ${t.divider}`, padding: '4px 10px', alignItems: 'center', gap: 6 }}>
                    <Cloud size={12} style={{ color: t.activeAccent }} />
                    <span style={{ fontSize: 11, fontWeight: 500, color: t.panelText, letterSpacing: 0.5 }}>{activeEnv === 'cloud' ? 'AWS BEDROCK' : 'LOCAL AI'}</span>
                </div>
            ...
            <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                ...
                {openPanel !== null && (
                    <div style={{
                        ...
                        {openPanel === 'upload' && (
                            <UploadPanel t={t} onUploadSuccess={handleUploadSuccess} />
                        )}
